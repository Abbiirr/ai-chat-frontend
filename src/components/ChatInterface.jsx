...
function buildDownloadLinks(payload) {
  ...
  if (Array.isArray(payload.created_files)) {
    payload.created_files.forEach((fullPath) => {
      ...
      links.push({
        name,
        url: `http://localhost:8000/download/?filename=${encodeURIComponent(
          name
        )}`,
        type: "trace_analysis",
      });
...
const clearStreamingFlags = (messages) =>
  messages.map((msg) =>
...
const handlers = {
  ...
  "Compiled Request Traces": createTextAppender(
    ...
    setMessages((prev) => {
      const updatedMessages = clearStreamingFlags(prev);
      ...
      <main className="messages-container">
        <div className="messages-wrapper">
          ...
          {messages.map((message, index) => {
            const key =
              message.id ??
              `${message.from}-${index}-${message.text?.slice(0, 20)}`;
            return (
              <ChatBubble
                key={key}
                message={message}
